...
class ExtractMarriott:
    ...
    def get_search_data(self, hotel_id, check_in_date, check_out_date, guest_count):
        ...
        hotel_name = hotel_id
        hotel_id = hotel_name.split('-')[0]

        _proxy_url = self._proxy_fetcher.fetch_proxy()
        if not _proxy_url:
            raise "ERROR-101(ProxyServerError) : Proxy url not retrieved from the server"
        logger.info("proxy IP retrieved for current session")
        ...
        if not hotel_id or length_of_stay <= 0 or guest_count <= 0:
            raise RuntimeError("ERROR-1003(HotelGroupException) : hotel_id/no_of_stays/guest must have some values.")
        ...
        # ---------- 2: JS page ----------
        ref_url = f"https://www.marriott.com/en-us/hotels/{hotel_name}/overview/"
        ...
        if match_phoenix_hws:
            ...
            print("phoenix_hws_signature", phoenix_hws_signature)

            # ---------- 3: Standard hotel form page ----------
        ...
        if "Invalid Property Code" in resp4.text:
            logging.error("SuccessError : Property Code is invalid.")
            return {
                "statusCode": resp4.status_code,
                "details": "Property Code is invalid."
            }

        # ---------- 4: Submit form page ----------
        url = f"https://www.marriott.com/reservation/availabilitySearch.mi?"\
              f"destinationAddress.country=&"\
              f"lengthOfStay={length_of_stay}&"\
              f"fromDate={check_in_mm}%2F{check_in_dd}%2F{check_in_yyyy}&"\
              f"toDate={check_out_mm}%2F{check_out_dd}%2F{check_out_yyyy}&"\
              f"numberOfRooms=1&"\
              f"numberOfAdults={guest_count}&"\
              f"guestCountBox={guest_count}+Adults+Per+Room&"\
              f"childrenCountBox=0+Children+Per+Room&"\
              f"roomCountBox=1+Rooms&"\
              f"childrenCount=0&"\
              f"childrenAges=&"\
              f"clusterCode=none&"\
              f"corporateCode=&"\
              f"groupCode=&"\
              f"isHwsGroupSearch=true&"\
              f"propertyCode={hotel_id.upper()}&"\
              f"useRewardsPoints=true&"\
              f"flexibleDateSearch=false&"\
              f"t-start={check_in_mm}%2F{check_in_dd}%2F{check_in_yyyy}&"\
              f"t-end={check_out_mm}%2F{check_out_dd}%2F{check_out_yyyy}&"\
              f"fromDateDefaultFormat={check_in_mm}%2F{check_in_dd}%2F{check_in_yyyy}&"\
              f"toDateDefaultFormat={check_out_mm}%2F{check_out_dd}%2F{check_out_yyyy}&"\
              f"fromToDate_submit={check_out_mm}%2F{check_out_dd}%2F{check_out_yyyy}&"\
              f"fromToDate="
        ...
        if match_PhoenixBookSearchProductsByProperty:
            ...
            print("signature_PhoenixBookSearchProductsByProperty", signature_PhoenixBookSearchProductsByProperty)


        # ---------- 6: Promotional Rate Page(actual page for points data) ----------
        ...
        if "\"Invalid Property Code\"" in resp10.text:
            logging.error("SuccessError : Property Code is invalid.")
            return {
                "statusCode": resp10.status_code,
                "details": "Property Code is invalid."
            }
        if '"code":"standard"' in resp10.text and '"code":"redemption"' not in resp10.text:
            logging.error("SuccessError : There are no redemption rates available for the dates you selected.")
            return {
                "statusCode": resp10.status_code,
                "details": "There are no redemption rates available for the dates you selected."
            }

        text_resp = resp10.text
        file_path = f"marriott_response.txt"

        logger.info(f"Saving API Response at Path:- {file_path}")
        with open(file_path, "a", encoding="utf-8") as f:
            f.write(str(resp10.status_code))
            f.write("\n")
        return None
...
if __name__ == "__main__":
    ...
    crawl.get_search_data("lastd-the-english-hotel-las-vegas-a-tribute-portfolio-hotel", "2026-02-15", "2026-02-17", 1)