...
class ExtractMarriott:
    ...
    def get_search_data(self, hotel_id, check_in_date, check_out_date, guest_count):
        ...
        #         break
        #     else:
        #         logger.warning(f"Attempt {attempt} failed to retrieve proxy URL")
        #         if attempt < MAX_RETRIES:
        #             time.sleep(RETRY_DELAY)
        # else:
        #     raise RuntimeError("ERROR-101(ProxyServerError) : Proxy url not retrieved from the server")
        ...
        if "The hotel is currently closed" in resp2.text:
            logging.error("ERROR-1004(HotelGroupException) : The hotel is currently closed.")
            return {
                "status_code": resp2.status_code,
                "details": "The hotel is currently closed."
            }
        if resp2.status_code != 200:
            logging.error("Error Occured.")
            raise RuntimeError("ERROR-1006 (PageError) : Error Occurred.")
...
if __name__ == "__main__":
    ...
    crawl.get_search_data("sellc", "2026-02-15", "2026-02-17", 1)